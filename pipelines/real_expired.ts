...
import { generateCalls as generateCallsSignedAttrs } from "../unrolled_leaves/signedAttrs_256";
...
import { DynSignedAttrs } from "../unrolled_leaves";
import { Digest_SignedAttrs_256_Methods } from "../unrolled_leaves/digestSignedAttrs";
import {
  Exp2048_Input,
  Exp2048_Output,
...
function generateCallDigestSignedAttrs(signedAttrs: Uint8Array): PerProgram {
  return {
    methods: Digest_SignedAttrs_256_Methods,
    calls: [
      {
        methodName: "digestSignedAttrs256",
        args: [DynSignedAttrs.fromBytes(signedAttrs)],
      },
    ],
  };
}
...
function getPipeline() {
  ...
  const callsPerProgram: PerProgram[] = [
    ...
    generateCallsSignedAttrs(bundle.signed_attrs, bundle.lds),
    generateCallDigestSignedAttrs(bundle.signed_attrs),
    generateCallLocalRsaExponentiate(
      bundle.signed_attrs,
      pubkeyLocal.modulus,
